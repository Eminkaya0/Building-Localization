...
Monte~Carlo experiments demonstrate that the MPC controller achieves $2.6\times$ lower RMSE than a reactive exponential baseline (\SI{0.42}{m} vs.\ \SI{1.08}{m}), circular trajectories yield $8\times$ better observability conditioning, and the EKF provides significant robustness to odometry drift---maintaining \SI{3.05}{m} accuracy where single-shot back-projection degrades to \SI{4.07}{m} under mild drift conditions.
\end{abstract}
...
% ── Table V: Drift ────────────────────────────────────────
\subsection{Odometry Drift Robustness}

\begin{table}[t]
\centering
\caption{RMSE under odometry drift (fixed $h{=}\SI{80}{m}$, 10 seeds).}
\label{tab:drift}
\begin{tabular}{@{}ccc@{}}
\toprule
$\sigma_{\text{pos}}$ \textbf{(m/$\sqrt{\text{s}}$)} & \textbf{Back-proj.\ (m)} & \textbf{EKF (m)} \\
\midrule
0 (ideal)  & \textbf{0.40} & 0.67 \\
0.02       & 4.07 & \textbf{3.05} \\
0.05       & 4.18 & \textbf{3.24} \\
0.10       & 4.59 & \textbf{3.62} \\
0.20       & 5.73 & \textbf{4.47} \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:drift} reveals the critical advantage of multi-measurement fusion under realistic conditions.
With perfect odometry, back-projection is competitive (\SI{0.40}{m} vs.\ \SI{0.67}{m}).
However, even mild drift ($\sigma_{\text{pos}} = \SI{0.02}{m/\sqrt{s}}$, corresponding to ${\sim}\SI{0.2}{m}$ RMS drift after \SI{120}{s}) causes back-projection to degrade by $10\times$ to \SI{4.07}{m}, while the EKF degrades only $4.5\times$ to \SI{3.05}{m}.
At severe drift ($\sigma_{\text{pos}} = 0.2$), the EKF maintains a 22\% advantage (\SI{4.47}{m} vs.\ \SI{5.73}{m}).
The EKF's robustness stems from averaging out pose errors across many measurements from diverse viewpoints, whereas back-projection relies on a single (drifted) pose.
...
\textbf{Drift robustness.}
Table~\ref{tab:drift} confirms that the EKF's multi-measurement fusion provides significant robustness to odometry drift.
Even mild drift ($\sigma_{\text{pos}} = 0.02$) causes a $10\times$ degradation in back-projection, while the EKF degrades only $4.5\times$.
This is the strongest practical argument for the EKF: real UAV odometry always drifts.
...
The drift model assumes isotropic random walk; real VIO systems exhibit structured drift patterns that could be modeled more accurately.
